...
(function() {
  var Funcco, _, buildMatchers, commander, configure, defaults, document, format, fs, getLanguage, highlightjs, languages, marked, parse, path, run, version, write,
    slice = [].slice;
  ...
  parse = function(source, code, config) {
    var annotFilter, codeText, docsText, hasCode, i, isText, j, k, lang, len, len1, line, lines, match, maybeCode, save, sections, typeAnnotations;
    if (config == null) {
      config = {};
    }
    lines = code.split('\n');
  ...
  format = function(source, sections, config) {
    var code, i, j, language, len, markedOptions, ref, results, section;
  ...
  configure = function(options) {
    ...
    config.sources = options.args.filter(function(source) {
      ...
      lang = getLanguage(source, config);
      if (!lang) {
        console.warn("funcco: skipped unknown type (" + (path.basename(source)) + ")");
      }
      return lang;
    }).sort();
    return config;
